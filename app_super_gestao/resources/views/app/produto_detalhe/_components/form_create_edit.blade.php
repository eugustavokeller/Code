...
@else
    <form method="post" action="{{ route('produto-detalhe.store') }}">
        @csrf
@endif
        <input type="text" value="{{ $produto_detalhe->produto_id ?? old('produto_id') }}" name="produto_id" placeholder="ID Produto" class="borda-preta">
        ...
        <select name="unidade_id">
            ...
            @foreach ($unidades as $unidade)
                <option value="{{ $unidade->id }}"
                    {{ ($produto_detalhe->unidade->id ?? old('unidade_id')) == $unidade->id ? 'selected' : '' }}>
